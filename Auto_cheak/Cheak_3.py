...
def period(n):
    ...
    try:          
        WhatisHomework = browser.find_elements(By.CLASS_NAME, 'item.state')
        Homework = browser.find_elements(By.CLASS_NAME, 'sche') 
        
        print("--------------------------------")
        print("< " + topic_list[n] + " 과제 탐색완료! >")
        
        for i in range(len(WhatisHomework)):
            if(WhatisHomework[i].text != "종료"):
                print("과제가 있습니다!")
                print(Homework[i].text); break
            else:
                print("과제가 없습니다."); break
        
        # WhatisHomework = browser.find_element(By.CLASS_NAME, 'article')
        # print("< " + topic_list[n] + " 과제가 있을 수도 있음! >\n" + WhatisHomework.text + '\n')
    except:
        print(topic_list[n] + " 과제가 없거나 Error" + '\n')    
